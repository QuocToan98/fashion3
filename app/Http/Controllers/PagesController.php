...
class PagesController extends Controller
{
    public function index()
    {
        ...
        $pc = DB::table('products')
                ...
                ->paginate(4);
        $slide = slide::all();
        $sp_sapramat = products::where('cat_id',10)->limit(4)->get();
        $new_product = products::where('status',1)->limit(4)->get();
        $tin_tuc = news::where('status',1)->limit(3)->get(); 
    	return view('homedemo',['tin_tuc'=>$tin_tuc,'pc'=>$pc,'slide'=>$slide,'sp_sapramat'=>$sp_sapramat,'new_product'=>$new_product]);
    }
    ...
    // update dev-toan
     ...
     public function getsearch(Request $req){
        $product = products::where('name','like','%'.$req->key.'%')
                            ...
                            ->get();
        return view('modules.search',compact('product'));
    }
    ...
    public function edit()
   {
    ...
    public function vechungtoi (){
        return view('modules.vechungtoi');
    }
    ...
    public function getcate($cat)
    {
    	if ($cat == 'vay') {
            ...
            $mobile = DB::table('products')
                ...
                ->select('products.*','pro_details.cpu','pro_details.ram','pro_details.screen','pro_details.vga','pro_details.storage','pro_details.exten_memmory','pro_details.cam1','pro_details.cam2','pro_details.sim','pro_details.connect','pro_details.pin','pro_details.os','pro_details.note')
                ->paginate(12);
    		return view('category.catsp',['data'=>$mobile]);
    	} 
        elseif ($cat == 'aosomi') {
            // mobile
            $lap = DB::table('products')
                ...
                ->select('products.*','pro_details.cpu','pro_details.ram','pro_details.screen','pro_details.vga','pro_details.storage','pro_details.exten_memmory','pro_details.cam1','pro_details.cam2','pro_details.sim','pro_details.connect','pro_details.pin','pro_details.os','pro_details.note')
                ->paginate(12);
            return view('category.catsp',['data'=>$lap]);
        }
        elseif ($cat == 'dodoi') {
            // mobile
        $pc = DB::table('products')
                ...
                ->select('products.*','pro_details.cpu','pro_details.ram','pro_details.screen','pro_details.vga','pro_details.storage','pro_details.exten_memmory','pro_details.cam1','pro_details.cam2','pro_details.sim','pro_details.connect','pro_details.pin','pro_details.os','pro_details.note')
                ->paginate(8);
            return view('category.catsp',['data'=>$pc]);
        }
        elseif ($cat == 'tin-tuc') {
            $new =  DB::table('news')
